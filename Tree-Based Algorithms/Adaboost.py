...
# The main and documented interactions files are loaded.
...
# Splicing and trimming the loaded datasets for machine learning.
...
# Setting up the Decision Tree for the AdaBoost Regressor. 
tree = DecisionTreeRegressor(max_depth = 300, criterion = 'friedman_mse', min_samples_leaf = 15, random_state = 1)
bag = AdaBoostRegressor(base_estimator = tree, n_estimators = 100, learning_rate = 10, loss = 'exponential')

# Fitting the Adaboost regressor.
m = bag.fit(X_train,y_train)

# Getting the probabilities and putting them into a datatable.
pred = cross_val_predict(m,X_test,y_test,cv = 10) 
