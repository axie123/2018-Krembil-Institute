...
dt =  DecisionTreeClassifier(max_depth = 800,criterion = 'entropy',random_state = 1)
rfg = RandomForestRegressor(n_estimators = 50, max_depth = 200, max_features = 1, min_samples_leaf = 0.00001, random_state = 1)
gb = GradientBoostingRegressor(n_estimators = 500,max_depth = 200, random_state = 1)

cl = [('LR', lrg),('DT', dt),('RF', rfg),('GB', gb)]

vc = VotingClassifier(estimators = cl)
m = vc.fit(X_train,y_train)
